...
import type { CanvasPersistedState } from '../types/canvasRuntime';
...
const isDataAudioSourceValue = (value?: string | null) => (
  /^data:audio\/[a-zA-Z0-9.+-]+;base64,/i.test(String(value || '').trim())
);

export const isDataMediaSourceValue = (value?: string | null) => (
  isDataImageSourceValue(value) || isDataVideoSourceValue(value) || isDataAudioSourceValue(value)
);
...
const stripLegacyAlchemy = (item: BufferItem): BufferItem => {
  if (!Object.prototype.hasOwnProperty.call(item, 'alchemy')) return item;
  const next = { ...item } as BufferItem & { alchemy?: unknown };
  delete next.alchemy;
  return next;
};
...
export const stripHeavyDataThumbnail = (item: BufferItem): BufferItem => {
  const nextItem = stripDataImageProvenance(stripLegacyAlchemy(item));
...
export const prepareCanvasWorkflowTemplateItem = (item: BufferItem): BufferItem => {
  ...
  if (next.thumbnail && isDataImageSourceValue(next.thumbnail) && next.thumbnail.length > DATA_THUMBNAIL_KEEP_MAX_CHARS) {
    next.thumbnail = undefined;
  }
  return next;
};
...
export const cloneDrawerValue = <T,>(value: T): T => {
  try {
